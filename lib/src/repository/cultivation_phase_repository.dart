...
class CultivationPhaseRepository extends Disposable {
  ...
  Future<Cultivation_PhaseModel> createCultivationPhase(bool state_phase,String name,String image ,String duration, String description,int productId) async {
    ...
    var data = await connection.mutation(query, variables: {"name": name,"image": image,"durarion": duration, "description" : description});
    var id = data["data"]["insert_cultivation_guide"]["returning"][0]["id"];
    return Cultivation_PhaseModel(id: id, name: name, image:image,duration: duration, description: description);
  }
  ...
  Future<Cultivation_PhaseModel> updateCultivationPhase(bool state_phase,String name,String image ,String duration, String description,int productId, int id) async {
    var query = """
    ...
    var data = await connection.mutation(query, variables: {"state_phase": state_phase,"name": name, "image":image,"duration": duration, "description" : description, "id_producto": productId});
    var id = data["data"]["update_cultivation_phase"]["returning"][0]["id"];
    return Cultivation_PhaseModel(id: id,state_phase: state_phase, name: name,image: image,duration: duration, description: description);
  }
  ...
  Stream<List<Cultivation_PhaseModel>> getCultivationPhase() {
    ...
    return snapshot.stream.map(
      (jsonList) => Cultivation_PhaseModel.fromJsonList(jsonList["data"]["cultivation_phase"]),
    );
  }
