...
class CultivationGuideRepository extends Disposable {
  ...
  Future<Plague_DiseaseModel> createPlagueDisease(String name,String image ,String damge ,String description) async {
    ...
    var data = await connection.mutation(query, variables: {"name": name,"image": image,"damge": damge, "description" : description});
    var id = data["data"]["insert_plague_disease"]["returning"][0]["id"];
    return Plague_DiseaseModel(id: id, name: name, image:image,damage: damge, description: description);
  }
  ...
  Future<Plague_DiseaseModel> updatePlagueDisease(String name,String image ,String damge ,String description, int id) async {
    var query = """
  ...
  Stream<List<Plague_DiseaseModel>> getPlagueDisease() {
    ...
    return snapshot.stream.map(
      (jsonList) => Plague_DiseaseModel.fromJsonList(jsonList["data"]["plague_disease"]),
    );
  }
